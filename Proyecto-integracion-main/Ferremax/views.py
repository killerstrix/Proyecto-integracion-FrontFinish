    ...
    if request.method == "POST":
        ...
        if nombre_usuario and contraseña:
            ...
            if usu is not None:
                ...
                messages.success(request, "Inicio de sesión con éxito")
...
def registro(request):
    if request.method == "POST":
        ...
        # Validaciones adicionales en el servidor por seguridad
        if len(nombre_usuario) < 8:
            return JsonResponse(
                {"error": "El nombre de usuario debe tener al menos 8 caracteres."}
            )
        if len(nombre_completo) < 10 or any(char.isdigit() for char in nombre_completo):
            return JsonResponse(
                {
                    "error": "El nombre completo debe tener al menos 10 caracteres y no contener números."
                }
            )
        if not (correo.endswith(".cl") or correo.endswith(".com")):
            return JsonResponse(
                {"error": "El correo electrónico debe terminar en .cl o .com."}
            )
